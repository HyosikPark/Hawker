...
- "테스트넷이면 가짜 아닌가?" → 맞다, 아직 실화폐가 아니다. 의도적 선택: 수요를 먼저
  검증하고 메인넷 스위치(env 3줄)를 올린다. 결제 코드 경로는 메인넷과 동일하며
  온체인 정산 tx를 공개해뒀다. 크레딧(법정화폐) 레일도 준비돼 있다.
- "SSRF는 막았나? (localhost를 upstream으로 등록하면?)" → 막았다. openapiUrl과
  upstreamBaseUrl 모두 루프백·사설대역·링크로컬(169.254 메타데이터 포함)·.internal을
  생성 시점에 차단한다 (ssrf.ts, 테스트 포함). 지적할 다른 구멍이 있다면 진심으로 환영.
- "판매자 키를 왜 너한테 줘?" → AES-256-GCM 암호화 저장이고 코드가 공개돼 있어 검증
  가능하다. 못 믿겠으면 AGPL이니 셀프호스팅해도 된다 — 그래도 우리 생태계에 남는다.
- "왜 AGPL?" → 1인 프로젝트라 대기업이 그대로 폐쇄형 SaaS로 감싸는 것만 방지했다.
  읽기·포크·셀프호스팅은 완전히 자유다.
- "HN 계정이 새것인데?" → (효식님 참고) 게시 전에 계정을 만들어 며칠간 다른 글에
  평범한 댓글 몇 개 달아두면 신뢰도에 도움. Show HN 규칙(news.ycombinator.com/showhn.html)
  핵심: 직접 만든 것, 방문자가 로그인 없이 써볼 수 있어야 함 — 우리는 무료 툴 curl로 충족.
